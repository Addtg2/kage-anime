...
- **Kodik iframe не принимает control postMessage снаружи** — попытки сменить серию через `postMessage` (форматы `kodik_player_change_episode` и др.) ничего не делают. Поэтому при смене серии в боковом пикере iframe пересоздаётся (key включает `epNum` в [components/kage/player-switcher.tsx](components/kage/player-switcher.tsx)). Озвучка, выбранная во **внутреннем UI Kodik**, не переживает смену серии — для устойчивого выбора используйте KAGE-селектор (иконка микрофона справа от кнопок плеера), он пишет в zustand и применяется к новому URL iframe.
