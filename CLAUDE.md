...
- **Kodik iframe не принимает control postMessage снаружи** — попытки сменить серию через `postMessage` (форматы `kodik_player_change_episode` и др.) ничего не делают. Поэтому при смене серии в баре серий iframe пересоздаётся (key включает `episode` в [components/kage/embedded-player.tsx](components/kage/embedded-player.tsx) — встроенный плеер в табе «Эпизоды» детальной страницы; отдельного роута `/watch` больше нет, он редиректит на `/anime/[id]`). Озвучка, выбранная во **внутреннем UI Kodik**, не переживает смену серии — для устойчивого выбора используйте KAGE-панель озвучек справа, она пишет в zustand и применяется к новому URL iframe. С зарегистрированным `KODIK_TOKEN` (env/Worker secret) `/search` отдаёт полный список озвучек → панель заполняется и нативный селектор Kodik скрывается (`withHiddenSelectors`).
